...
_dbglog("run_panel_workflow1.py:start", "Script started", {"anime_image": args.anime_image, "output": output_image_path})

# ---- STEP 2: Setup Server and Client ID ----
server_address = os.getenv("COMFYUI_URL")
_dbglog("run_panel_workflow1.py:env", "COMFYUI_URL value", {"COMFYUI_URL": server_address or "NOT SET"})
...
if _runpod_key:
    ...
    _dbglog("run_panel_workflow1.py:runpod", "RunPod API key set", {})
...
def queue_prompt(prompt):
    ...
    try:
        ...
        body = resp.read()
        _dbglog("run_panel_workflow1.py:queue_prompt:ok", "Prompt queued", {"status": resp.status, "body": body.decode()[:300]})
        return json.loads(body)
    except urllib.error.HTTPError as e:
        err_body = e.read().decode(errors="replace")[:2000]
        _dbglog("run_panel_workflow1.py:queue_prompt:FAIL", "ComfyUI rejected prompt", {"status": e.code, "body": err_body})
        raise
...
def upload_image(filepath):
    ...
    _dbglog("run_panel_workflow1.py:upload_image", "Uploaded image to ComfyUI", {"result": result, "path": filepath})
    return result["name"]
...
def get_images(ws, prompt):
    ...
    _dbglog("run_panel_workflow1.py:get_images:start", "Waiting for WS messages", {"prompt_id": prompt_id})
        ...
        if isinstance(out, str):
            ...
            if message['type'] == 'executing':
                ...
                if data['node'] is None and data['prompt_id'] == prompt_id:
                    _dbglog("run_panel_workflow1.py:get_images:done", "Workflow execution complete", {})
                    break
        ...
        if 'images' in node_output:
            for image in node_output['images']:
                if image.get('type') == 'temp':
                    continue
                _dbglog("run_panel_workflow1.py:get_images:downloading", "Downloading output image", {"node_id": node_id, "filename": image['filename']})
...
workflow_path = os.path.join(BASE_DIR, "character1.json")
_dbglog("run_panel_workflow1.py:workflow_json", "Loading workflow", {"path": workflow_path, "exists": os.path.exists(workflow_path)})
...
prompt["4"]["inputs"]["image"]  = main_pose_name
prompt["2"]["inputs"]["text"]   = args.main_prompt
prompt["14"]["inputs"]["image"] = anime_image_name

# ---- STEP 5: Connect WebSocket, Run Workflow ----
ws_address = server_address.replace("https://", "wss://", 1).replace("http://", "ws://", 1)
_dbglog("run_panel_workflow1.py:connecting_ws", "Connecting WebSocket", {"ws_address": ws_address})
ws = websocket.WebSocket()
ws.connect(f"{ws_address}/ws?clientId={client_id}")
_dbglog("run_panel_workflow1.py:ws_connected", "WebSocket connected", {})
images = get_images(ws, prompt)
ws.close()
_dbglog("run_panel_workflow1.py:workflow_done", "Workflow completed", {"num_nodes": len(images)})
    ...
    for image_data in images[node_id]:
        if image_data:
            ...
            with open(output_image_path, 'wb') as f:
                f.write(image_data)
            _dbglog("run_panel_workflow1.py:saved", "Saved output image", {"path": output_image_path, "size": len(image_data)})
            saved = True
            break
    if saved:
        break
