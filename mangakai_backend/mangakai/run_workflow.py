...
# Parse full URL like: https://xxxx.ngrok-free.app
u = urlparse(server_address)
if u.scheme not in ("http", "https") or not u.netloc:
    raise ValueError(f"Invalid COMFYUI_URL: {server_address}")

BASE_HTTP = f"{u.scheme}://{u.netloc}"          # https://xxxx.ngrok-free.app
BASE_WS   = f"{'wss' if u.scheme=='https' else 'ws'}://{u.netloc}"  # wss://xxxx...

def queue_prompt(prompt):
    p = {"prompt": prompt, "client_id": client_id}
    data = json.dumps(p).encode("utf-8")
    req = urllib.request.Request(f"{BASE_HTTP}/prompt", data=data)
    return json.loads(urllib.request.urlopen(req).read())

def get_image(filename, subfolder, folder_type):
    data = {"filename": filename, "subfolder": subfolder, "type": folder_type}
    url_values = urllib.parse.urlencode(data)
    with urllib.request.urlopen(f"{BASE_HTTP}/view?{url_values}") as response:
        return response.read()

def get_history(prompt_id):
    with urllib.request.urlopen(f"{BASE_HTTP}/history/{prompt_id}") as response:
...
script_path = os.path.join(HERE, "face_to_anime (1).json")
...
ws.connect(f"{BASE_WS}/ws?clientId={client_id}")
